...
.DESCRIPTION
    ...
    needing a running Editor:
      ...
      3. Every approved requirement with trace: required is referenced as
         SWR.SWR_<n> somewhere in Assets/**/*.cs (approximation of the [Traces]
         reflection check; the authoritative check runs in the Editor/EditMode tests).
...
try {
    ...
    foreach ($f in $files) {
        ...
        $reqs += [pscustomobject]@{
            ...
            SourcePath = $rel; TraceRequired = $traceRequired
    ...
    if ($errors.Count -eq 0) {
        $globalHash = Get-ShortHash (($reqs | ForEach-Object { "$($_.Id)|$($_.Status)|$($_.TraceRequired)|$($_.Title)|$($_.SourcePath)|$($_.ContentHash)" }) -join "`n")
        ...
        foreach ($r in $reqs) {
            ...
            $null = $sb.Append("        [Requirement(`"$($r.Id)`", RequirementStatus.$($r.Status), `"$csTitle`", `"$($r.SourcePath)`", $traceLit, `"$($r.ContentHash)`")]`n")
    ...
    # --- 3. Approved requirements with a required trace must be referenced in code ---
    $tracedNumbers = @{}
    Get-ChildItem (Join-Path $root "Assets") -Recurse -Filter *.cs |
        Where-Object { $_.Name -notlike "*.g.cs" } |
        Select-String -Pattern "SWR\.SWR_(\d+)" -AllMatches |
        ForEach-Object { $_.Matches } |
        ForEach-Object { $tracedNumbers[[int]$_.Groups[1].Value] = $true }

    foreach ($r in $reqs) {
        if ($r.Status -eq "Approved" -and $r.TraceRequired -and -not $tracedNumbers.ContainsKey($r.Number)) {
            $errors += "[ReqToCode] $($r.Id) `"$($r.Title)`" is approved but not traced in code. Add [Traces(SWR.$($r.Id.Replace('-', '_')))] to the implementing code element (source: $($r.SourcePath))."
        }
    }
